...
class RawClientTCP:
    ...
    def craft_tcp_header(self, tcp_seq=454, tcp_ack_seq=0, tcp_doff=5, tcp_fin=0, tcp_syn=0, tcp_rst=0,
        ...
        tcp_off = (tcp_doff << 4) | 0

        # !->network, H->2 bytes, L->4 bytes, B->1 bytes
        tcp_header = pack('!HHLLBBHHH', self.source_port, self.dest_port, tcp_seq, tcp_ack_seq, tcp_off, tcp_flags, tcp_window, 0, tcp_urg_ptr)
        ...
        tcp_length = 20 + len(self.msg)

        pseudo_header = pack('!4s4sBBH', source_address, dest_address, placeholder, protocol, tcp_length)
        pseudo_header = pseudo_header + tcp_header + self.msg

        tcp_checksum = self.calculate_checksum(pseudo_header)
        # print(tcp_checksum)
        # Make the TCP header again and fill the correct checksum
        tcp_header = pack('!HHLLBBH', self.source_port, self.dest_port, tcp_seq, tcp_ack_seq, tcp_off, tcp_flags, tcp_window) + pack('H', tcp_checksum) + pack('!H', tcp_urg_ptr)
    ...
    def craft_ack_packet(self):
        ...
        self.craft_tcp_header(tcp_seq=self.seq_number, tcp_ack_seq=self.ack_number, tcp_ack=1)
    ...
    def craft_msg_packet(self, msg):
        ...
        """
        self.msg = msg.encode()
        self.craft_tcp_header(tcp_seq = self.seq_number, tcp_ack_seq = self.ack_number, tcp_ack=1, tcp_psh=1)
        self.packet_to_send = self.ip_header + self.tcp_header + self.msg
